...
const ChatWindow = ({ messages, onSendMessage }) => {
    ...
    return (
        <div className="flex h-full">
            <div className="w-2/4 bg-gray-100 border-r border-gray-300">
                <div className="p-4 font-bold">Chats</div>
                {messages.map((msg, index) => (
                    <div
                        key={index}
                        className={`p-2 hover:bg-gray-200 cursor-pointer ${activeChat === index ? 'bg-gray-200' : ''}`}
            ...
            <div className="flex-1 flex flex-col">
                <div className="flex-1 overflow-auto p-4">
                    {messages.map((msg, index) => (
                        <div key={index} className={`p-2 my-2 rounded-lg ${msg.sender === 'vendor' ? 'bg-blue-200 self-end' : 'bg-green-200'}`}>
                            {msg.text}
                        </div>
                    ))}
                </div>
